...
static void dump_tensor_attr(rknn_tensor_attr *attr) {
    ...
    printf("  index=%d, name=%s, n_dims=%d, dims=[%s], n_elems=%d, size=%d, w_stride=%d, size_with_stride=%d, "
           ...
           "zp=%d, scale=%f\n",
           attr->index, attr->name, attr->n_dims, dims, attr->n_elems, attr->size, attr->w_stride,
...
int init_yolo11_model(const char *model_path, rknn_app_context_t *app_ctx) {
    ...
    printf("[ZERO-COPY] Initializing YOLO11 model with Zero-Copy mode...\n");
    ...
    if (output_native_attrs[0].qnt_type == RKNN_TENSOR_QNT_AFFINE_ASYMMETRIC &&
        ...
        printf("[ZERO-COPY] Model is quantized (INT8)\n");
    } else {
        app_ctx->is_quant = false;
        printf("[ZERO-COPY] Model is NOT quantized (FP16)\n");
    }
    ...
    printf("[ZERO-COPY] ✅ Model initialized successfully!\n");
    return 0;
}

int NC1HWC2_i8_to_NCHW_i8(const int8_t *src, int8_t *dst, int *dims,
...
int release_yolo11_model(rknn_app_context_t *app_ctx) {
    ...
    printf("[ZERO-COPY] Releasing YOLO11 model resources...\n");
    ...
    printf("[ZERO-COPY] ✅ All resources released successfully!\n");
    return 0;
}

int inference_yolo11_model(rknn_app_context_t *app_ctx, image_buffer_t *img,
    ...
    if (dst_img.virt_addr == NULL && dst_img.fd == 0) {
        printf("[ZERO-COPY] Input memory buffer is invalid!\n");
        return -1;
    }
    ...
    printf("[ZERO-COPY] Running inference...\n");
    ...
    for (uint32_t i = 0; i < app_ctx->io_num.n_output; i++) {
        ...
        int   h       = app_ctx->output_attrs[i].n_dims > 2 ?
                        app_ctx->output_attrs[i].dims[2] : 1;
        int   w       = app_ctx->output_attrs[i].n_dims > 3 ?
                        app_ctx->output_attrs[i].dims[3] : 1;
        ...
        if (app_ctx->is_quant) {
            ...
            printf("[ZERO-COPY] ❌ Currently zero-copy does not support FP16! "
                   "Please use quantized model.\n");
            goto out;
        }
    }
